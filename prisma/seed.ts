...
async function createPumpkinPieRecipe(submitter: { id: string }) {
  ...
  const pieIngredientNames = pieIngredientsObj.map(ingredient => ingredient.name);
  const pieIngredients = await Promise.all(pieIngredientNames.map(async (name) => {
  ...
  const pieTags = await Promise.all(pieTagNames.map(async (name) => {
  ...
  await Promise.all(pieIngredients.map(ingredient => {
    const ingredientObj = pieIngredientsObj.find(obj => obj.name === ingredient.name);
...
async function createCaesarSaladRecipe(submitter: { id: string }) {
  ...
  const saladIngredientNames = saladIngredientsObj.map(ingredient => ingredient.name);
  const saladIngredients = await Promise.all(saladIngredientNames.map(async (name) => {
    const ingredient = await prisma.ingredient.findUnique({ where: { name } });
    if (ingredient) {
      return ingredient;
    }
  ...
  const saladTags = await Promise.all(saladTagNames.map(async (name) => {
    const tag = await prisma.tag.findUnique({ where: { name } });
    if (tag) {
      return tag;
    }
  ...
  const saladRecipe = await prisma.recipe.create({
    data: {
      ...
      preparationSteps: '["In a large wooden salad bowl, rub the inside with the garlic clove, then discard. Add the lettuce, croutons, and cheese. Set aside.","In a small bowl, whisk together the lemon juice, olive oil, salt, pepper, egg yolk, mustard, and anchovy paste. Pour over the lettuce in the salad bowl, and toss well.","Serve immediately."]',
      source: 'NYTimes Cooking',
      sourceUrl: 'https://cooking.nytimes.com/recipes/1017937-caesar-salad',
      submittedBy: submitter.id,
    }
  })
  await Promise.all(saladTags.map(tag => {
    return prisma.recipeTag.create({
      data: {
  ...
  await Promise.all(saladIngredients.map(ingredient => {
    const ingredientObj = saladIngredientsObj.find(obj => obj.name === ingredient.name);
    return prisma.recipeIngredient.create({
      data: {
        recipeId: saladRecipe.id,
        ingredientId: ingredient.id,
